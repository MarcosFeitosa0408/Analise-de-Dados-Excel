#  📊Análise de Dados - Excel
##  Orçamento Familiar

Este projeto demonstra técnicas de análise de dados utilizando o Microsoft Excel, uma ferramenta amplamente utilizada para manipulação e interpretação de dados. Através de exemplos práticos, são abordadas funcionalidades como Tabelas Dinâmicas, Gráficos, Fórmulas e Suplementos, proporcionando uma compreensão sólida das capacidades analíticas do Excel.

###  🎯 Objetivo
Capacitar usuários a:

Importar e organizar dados de diferentes fontes;

Aplicar funções e fórmulas para análise estatística;

Criar Tabelas Dinâmicas para resumir informações;

Gerar gráficos para visualização de padrões e tendências;

Utilizar suplementos como o "Ferramentas de Análise" para análises avançadas.

###  🛠️ Tecnologias Utilizadas
Microsoft Excel


Suplemento "Ferramentas de Análise"

Power Query (opcional)
Microsoft Create

###  📸 Demonstração
[Planilha_Orcamento_Domestico.xlsx](https://github.com/user-attachments/files/20024485/Planilha_Orcamento_Domestico.xlsx)


###  🚀 Como Usar
Abra o arquivo Excel fornecido no repositório.

Navegue pelas abas para explorar diferentes análises.

Para utilizar o suplemento "Ferramentas de Análise":

Acesse Arquivo > Opções > Suplementos.

Em Gerenciar, selecione Suplementos do Excel e clique em Ir.

Marque a opção Ferramentas de Análise e clique em OK.

Explore as funcionalidades aplicadas em cada aba para entender as técnicas utilizadas.


